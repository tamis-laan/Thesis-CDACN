...
void DACN3p::step(const vector<Window> &batch)
{
	//Allocate buffers
	...
	thrust::copy(rbuffer.begin(),rbuffer.end(),critic.input(0).data(2).begin());
	...
	thrust::copy(actor_n.output(0).data(1).begin(),actor_n.output(0).data(1).end(),critic.input(1).data(2).begin());

	//Forward Critic
	critic.forward(1); critic.forward(2);
	...
	thrust::copy(critic.output(0).data(2).begin(),critic.output(0).data(2).end(),qr.begin());
	for(int i=0; i<batch_size_; i++)
	{
		//Get the reward 
		...
		// critic_delta[i] = r+discount_*qr[i]*(1.0-t)-ql[i]); 
	}
