...
namespace ddnet
{
	struct rmsprop_kernel
	{
		const float learning_rate;
		const float decay_1;
		const float decay_2;
		const float epsilon;
		rmsprop_kernel(float learning_rate, float decay_1, float decay_2, float epsilon) : learning_rate(learning_rate), decay_1(decay_1), decay_2(decay_2), epsilon(epsilon) {}
		...
		__device__ void operator()(Tuple t)
		{
			...
			//Set weights
			thrust::get<3>(t) = thrust::get<3>(t) + learning_rate*thrust::get<1>(t)/(sqrt(thrust::get<2>(t))+epsilon);
		}
	};

	void RMSPROP::step(Adapter &adaptor, cudaStream_t stream)
	{
		for(int i=0; i<adaptor.size(); i++)
		{
			...
			thrust::for_each(	thrust::make_zip_iterator(thrust::make_tuple(adaptor.gradients(i).begin(), adaptor.buffers(0,i).begin(), adaptor.buffers(1,i).begin(), adaptor.weights(i).begin() ) ),
								...
								rmsprop_kernel(meta_parameters_[0],meta_parameters_[1],meta_parameters_[2],meta_parameters_[3]));
		}
	}	
}